...
AGENT_INPUT_TOKENS = Counter(
    ...
    "Total input tokens used by the agent",
)
AGENT_OUTPUT_TOKENS = Counter(
    "agent_output_tokens_total",
    "Total output tokens generated by the agent",
)
...
async def chat(request: ChatRequest):
    ...
    start = time.time()
    try:
        result = await run_agent(lc_messages)
        AGENT_CHAT_REQUESTS.labels(status="success").inc()
        AGENT_INPUT_TOKENS.inc(result["tokens_used"].input)
        AGENT_OUTPUT_TOKENS.inc(result["tokens_used"].output)
        return ChatResponse(**result)
    except Exception:
        AGENT_CHAT_REQUESTS.labels(status="error").inc()
        raise
    finally:
        AGENT_CHAT_LATENCY.observe(time.time() - start)
